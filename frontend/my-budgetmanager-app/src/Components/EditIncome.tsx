...
import { getIncomeById, updateIncome } from '../Api/ApiIncomes';
...
const EditIncome: React.FC = () => {
  ...
  useEffect(() => {
    getIncomeSources().then(data => setSources(data || []));
    if (id) {
      getIncomeById(Number(id)).then(data => {
        setDescription(data.income_description || '');
        setAmount(data.amount ? String(data.amount) : '');
        setSource(data.income_source_id ? String(data.income_source_id) : '');
        setDate(data.income_date ? data.income_date.slice(0, 10) : getToday());
        setLoading(false);
      });
    }
  }, [id]);
  ...
  return (
    <div className={styles.addIncomePage}>
      <h2 className={styles.title}>עריכת הכנסה</h2>
